...
function buildAddress(row: Row) {
  const parts = [
    ...
    nullableText(row["Numero"] ?? row["N?mero"]),
...
function normalizeCustomer(row: Row, index: number): NormalizedCustomer {
  const phone = nullableText(row["Tel?fono celular"]);
  const alternatePhone = nullableText(row["Tel?fono fijo"]);
  const iva = nullableText(row["IVA"]);
  const cuitParts = [
    nullableText(row["PrefijoCUIT"]),
    nullableText(row["CUIT"]),
    nullableText(row["D?gitoVerificador"])
  ].filter(Boolean);
