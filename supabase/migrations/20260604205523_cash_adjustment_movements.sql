...
as $$
  select
    coalesce(sum(case when tipo in ('venta', 'reparacion', 'facturacion', 'ajuste_ingreso') then monto else 0 end), 0)::numeric as ingresos,
    coalesce(sum(case when tipo in ('gasto', 'sueldo', 'ajuste_egreso') then monto else 0 end), 0)::numeric as egresos,
    (
